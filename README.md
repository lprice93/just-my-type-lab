...
* Write jQuery code to set up keyboard toggling:
  * While the shift key is held down, hide the lowercase keyboard and show the uppercase one (.keydown)
  * When the shift key is released, show the lowercase keyboard and hide the uppercase one (.keyup) -->
<!-- * When keys are pressed, they should be highlighted in the browser. (.keypress)
